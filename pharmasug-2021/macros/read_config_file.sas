...
%macro read_config_file(config_file=, sections=);
  ...
  data _null_;
    ...
    if ksubstr(left(cfg_line), 1, 1) not in ('#' '!' '*') and length(strip(cfg_line))>1;
    ...
    call symputx(_mac_var,strip(_mac_var_value),'g');
